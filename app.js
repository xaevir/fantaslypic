...
app.configure('production', function(){
  ...
  db = mongo.db("localhost/fantasly?auto_reconnect=true", {safe: true, strict: false}
  )
})

app.configure('staging', function(){
  app.set('port', process.env.PORT || 8041);
  db = mongo.db("localhost/dev_fantasly?auto_reconnect=true", {safe: true, strict: false})
})
...
app.post('/landing', function(req, res, next) {
  var html  = '<p>fantasy: '+req.body.fantasy+'</p>'
  ...
  db.fantasies.insert(req.body, function(err, doc) {
    if (err)
